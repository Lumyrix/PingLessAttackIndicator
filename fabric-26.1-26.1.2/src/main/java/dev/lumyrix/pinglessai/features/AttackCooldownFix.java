...
public class AttackCooldownFix {
    ...
    public static Multimap<Holder<Attribute>, AttributeModifier> getAttackSpeed(@NotNull Player player) {
        ...
        attackSpeed.put(Attributes.ATTACK_SPEED,
            new AttributeModifier(Identifier.fromNamespaceAndPath("pinglessai", "pingless_base"),
                base, AttributeModifier.Operation.ADD_VALUE));
        attackSpeed.put(Attributes.ATTACK_SPEED,
            new AttributeModifier(Identifier.fromNamespaceAndPath("pinglessai", "pingless_mult_base"),
                modifier, AttributeModifier.Operation.ADD_MULTIPLIED_BASE));
        attackSpeed.put(Attributes.ATTACK_SPEED,
            new AttributeModifier(Identifier.fromNamespaceAndPath("pinglessai", "pingless_mult_total"),
                modifierFinal, AttributeModifier.Operation.ADD_MULTIPLIED_TOTAL));

        if (player.hasEffect(MobEffects.HASTE)) {
            attackSpeed.put(Attributes.ATTACK_SPEED,
                new AttributeModifier(Identifier.fromNamespaceAndPath("pinglessai", "pingless_haste"),
                    (Objects.requireNonNull(player.getEffect(MobEffects.HASTE)).getAmplifier() + 1) * 0.1,
                    AttributeModifier.Operation.ADD_MULTIPLIED_TOTAL));
        }

        if (player.hasEffect(MobEffects.CONDUIT_POWER)) {
            attackSpeed.put(Attributes.ATTACK_SPEED,
                new AttributeModifier(Identifier.fromNamespaceAndPath("pinglessai", "pingless_conduit"),
